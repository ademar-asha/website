...
- `npm run deploy` - выполняет команду build, переносит или обновляет собранный проект (папку build) в ветке gh-pages (создаёт её, если она не существует), пушит ветку gh-pages (на gh настраиваем деплой из ветки gh-pages, деплоим этой командой)
